...
class FarkleConsoleGame(Farkle):
    ...
    def start_new_game(self):
        clear_screen()
        self.num_players = number_input("How many players?", 4,
                                        "Invalid Number (0 to quit).")
    ...
    def player_turn(self, player_id):
        ...
        while not busted and not stop:
            ...
            if p.current_points > 0:
                if inv_yn_input("Do you want to keep rolling {0} dice?"
                                .format(num_dice)):
                    stop = True
                    break
            ...
            for s in scores:
                ...
                i += 1
            choice = list_choice_input("What do you want to keep?",
                                       choices, default)
        ...
        if busted:
            print("{0} busted and drops {1} points! No points this turn. Score: {2}"
                  .format(p.name, p.current_points, p.score))
            p.current_points = 0
        elif stop:
            p.score += p.current_points
            if p.score < self.minimum_score:
                print("{0} is done with {1} points, but that's not enough to get on the board ({2}). Still no score!"
                      .format(p.name, p.current_points, self.minimum_score))
            else:
                print("{0} is done with {1} points. New score: {2}"
                      .format(p.name, p.current_points, p.score))
            p.current_points = 0
